...
if __name__ == '__main__':
    ...
    try:
        error_col = [sheet.cell_value(0, i) for i in range(sheet.ncols)].index("ErrorCode")
    except ValueError:
        print("Creating ErrorCode Column")
        error_col = sheet.ncols
        sheet_write.write(0, error_col, "ErrorCode")
    ...
    for i in range(1, sheet.nrows):
        ...
        if sheet.cell_value(i, response_col) == "":
            print("sending sms to " + str(liste[1]))
            response = send_sms(formatted)
            storedProc = "EXEC Educube.dbo.insert_send_sms_from_excel @sent_message=" + liste[
                2] + ", @mobile_no=" + liste[1] + ",@guid='" + response.split('"')[1] +"',@status='',@response=''"
            cursor.execute(storedProc)
            if response[0] == 'E':
                sheet_write.write(i, error_col, response)
            else:
                sheet_write.write(i, response_col, response)
    del sheet
    del workbook
